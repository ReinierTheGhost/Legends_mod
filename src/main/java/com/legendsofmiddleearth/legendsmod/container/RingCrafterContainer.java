package com.legendsofmiddleearth.legendsmod.container;
...
public class RingCrafterContainer extends Container {

	private final TileEntity tileEntity;
	private final PlayerEntity playerEntity;
	private final IItemHandler playerInventory;

	public RingCrafterContainer(int windowId, World world, BlockPos pos, PlayerInventory playerInvemtory,
			PlayerEntity player) {
		super(ContainerLoader.RING_CRAFTER_CONTAINER.get(), windowId);
		this.tileEntity = world.getBlockEntity(pos);
		playerEntity = player;
		this.playerInventory = new InvWrapper(playerInvemtory);
		...
		if (tileEntity != null) {
			tileEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(h -> {
				...
				addSlot(new SlotItemHandler(h, 3, 30, 17));
				addSlot(new SlotItemHandler(h, 4, 48, 35));
				addSlot(new SlotItemHandler(h, 5, 66, 53));
				addSlot(new SlotItemHandler(h, 6, 30, 17));
				addSlot(new SlotItemHandler(h, 7, 48, 35));
				addSlot(new SlotItemHandler(h, 8, 66, 53));
				addSlot(new SlotItemHandler(h, 9, 30, 104));
	...
	public boolean stillValid(PlayerEntity playerIn) {
		...
		return stillValid(IWorldPosCallable.create(tileEntity.getLevel(), tileEntity.getBlockPos()), playerIn,
				BlockLoader.RING_CRAFTER_AIRRUNE.get());
	}
